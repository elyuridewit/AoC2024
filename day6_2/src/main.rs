...
struct GuardMap {
    ...
    turn_points:    Vec<(usize, usize)>,  // The amount of turns currently taken
    loops_found:    usize,                // Amount of loops found     
}
...
impl GuardMap {
    fn walk_map(&mut self) {
        ...
        while !exit {
            ...
            // loop candidate checking
            let cur_col: Vec<&char> = self.map.column(self.guardx).into_iter().collect();
            let cur_row: Vec<&char> = self.map.row(self.guardy).into_iter().collect();
            println!("{:?}", cur_col);
            println!("{:?}", cur_row);
            if cur_col.contains(&&'+') && cur_row.contains(&&'+') {
            ...
            if next_obj == '#' {
                ...
                self.map[(self.guardy, self.guardx)] = '+';
                self.turn_points.push((self.guardy, self.guardx));
            } else {
                if self.map[(self.guardy, self.guardx)] != '+' {
...
fn main() {
    ...
    println!("Loops found: {:?}", map.loops_found)
